...
- T4 candidate bounded full-stream stdout/stderr evidence: complete stream
  SHA-256 digests and byte counts are computed while retaining only the bounded
  preview; receipt output digests now bind the complete streams. Total lifecycle
  deadline and native fault-path work remains pending.
