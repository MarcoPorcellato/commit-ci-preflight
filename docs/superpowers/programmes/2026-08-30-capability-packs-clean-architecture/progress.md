...
- M2 closure evidence: legacy `m2-manifest.json` schema `1.0` remains byte-for-byte preserved but is invalid against its declared base: three path byte/SHA-256 claims disagree with the raw blobs. `m2-manifest-v1.1.json` is the immutable corrected record for the same 18 sorted paths and base commit. Tests preserve and reject legacy v1.0, then positively verify only v1.1 from raw Git objects; neither test reads manifested bytes from the evolving checkout.
