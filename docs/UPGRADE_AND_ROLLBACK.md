...
## Prerelease candidate artifact verification

Before replacing a binary with a selected prerelease candidate, verify all four
properties from the extracted candidate archive:

1. `SHA256SUMS` matches the archive byte-for-byte.
2. `RELEASE_MANIFEST.json` names the expected selected release label.
