...
pub struct LegacyMatrixDigestBasisV1 {
...
impl LegacyMatrixDigestBasisV1 {
    pub fn outer_digest(&self) -> Result<String, MatrixContractError> {
        canonical_digest(&self.plan).map_err(MatrixContractError::Receipt)
    }
    pub fn runtime_digest(&self, id: &str) -> Result<&str, MatrixContractError> {
...
pub fn project_legacy_basis(
    ...
    let receipt = LegacyNormalizedReceipt {
        output: plan.receipt.output.clone(),
        freshness_seconds: plan.receipt.freshness_seconds,
    };
    let caches = plan
        .caches
        .iter()
        .map(|c| LegacyNormalizedCache {
            id: c.id.clone(),
    ...
    for r in &plan.runtimes {
        let runtime = LegacyNormalizedRuntime {
            ...
            cpu_count: r.runtime.cpu_count,
            memory_mib: r.runtime.memory_mib,
            pids_limit: r.runtime.pids_limit,
            network: r.runtime.network,
        };
        let checks = r
            .checks
            .iter()
            ...
            .collect::<Vec<_>>();
        let basis = LegacyExecutionPlanV1 {
        ...
        let d = canonical_digest(&basis).map_err(MatrixContractError::Receipt)?;
        digests.insert(r.id.clone(), d.clone());
